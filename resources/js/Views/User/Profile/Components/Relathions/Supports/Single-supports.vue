<template>
    <tr v-if="support.user">
        <td class="d-flex align-items-center" style="min-width: max-content">
            <a :href="`/${support.user.username}/Channel/Activity`">
                <img :src="support.user.avatar" class="rounded-circle " style="width: 4rem" />
            </a>
            <div class="d-flex flex-column align-items-start ml-3">
                <span class="font-weight-bold">Name</span>
                <label>
                    {{support.user.artistic_name}}
                    <img src="/images/icons/check.svg" alt="" class="icon-check ml-1" v-if="support.user.verification_date">
        ...
        <td class="mx-3">
            <div class="d-flex flex-column align-items-start">
                <span class="font-weight-bold">Monthly Amount:</span>
                <span>$ {{support.monthly_amount}}</span>
            </div>
        </td>
        <td>
            <div class="d-flex flex-column align-items-start">
                <span class="font-weight-bold">Start Date:</span>
                <span>{{support.start_date}}</span>
            </div>
        </td>
        <td>
