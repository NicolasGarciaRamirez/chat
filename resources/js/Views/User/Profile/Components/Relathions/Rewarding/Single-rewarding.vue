<template>
    <tr  v-if="rewarded.rewarding">
        <td class="d-flex align-items-center" style="min-width: max-content">
            <a :href="`/${rewarded.rewarding.username}/Channel/Activity`">
                <img :src="rewarded.rewarding.avatar" class="rounded-circle " style="width: 4rem" />
            </a>
            <div class="d-flex flex-column align-items-start ml-3">
                <label class="font-weight-bold">Name</label>
                <label>
                    {{rewarded.rewarding.artistic_name}}
                    <img src="/images/icons/check.svg" alt="" class="icon-check ml-1" v-if="rewarded.rewarding.verification_date">
        ...
        <td class="mx-3">
            <div class="d-flex flex-column align-items-start">
                ...
                <span>${{rewarded.amount}}</span>
            </div>
        </td>
        <td>
            <div class="d-flex flex-column align-items-start">
                <label class="font-weight-bold">Date:</label>
                <span>{{rewarded.date}}</span>
            </div>
        </td>
        <td>
