<template>
    <tr v-if="reward.user">
        <td class="d-flex align-items-center" style="min-width: max-content">
            <a :href="`/${reward.user.username}/Channel/Activity`">
                <img :src="reward.user.avatar" class="rounded-circle " style="width: 4rem" />
            </a>
            <div class="d-flex flex-column align-items-start ml-3">
                <label class="font-weight-bold">Name</label>
                <label>
                    {{reward.user.artistic_name}}
                    <img src="/images/icons/check.svg" alt="" class="icon-check ml-1" v-if="reward.user.verification_date">
