<template>
    <div class="modal fade modal-share" tabindex="-1" role="dialog" aria-labelledby="ModalShare" aria-hidden="true" id="ModalShare" ref="ModalShare">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content modal-border-white p-3 mx-auto" style="max-width: 650px;">
                ...
                <div class="modal-body p-1" v-if="post">
                    <textarea class="form-control mb-3" cols="30" rows="10" v-model="post.description"></textarea>
                    <div class="divider w-100"></div>
                    <div class="content-icons">
                        <div>
                            <ShareNetwork
                                network="facebook"
                                :url="`https://www.noisesharks.com/Post/${post.token}`"
                    ...
                    <form class="fild-comment">
                        <input type="text" :id="`shared${post.token}`" :value="`https://www.noisesharks.com/Post/${post.token}`" class="form-control">
    ...
    methods:{
        saveShare(){
            axios.post(`/Share/store/${this.post.id}`).then(res =>{
                if(res.data.saved) this.post.shares.push(res.data.share)
            }).catch(err =>{console.log(err)})
        },
        copyLink(){
            let copyText = document.getElementById("shared"+this.post.token);
            copyText.select();
